...
def recieveData():
    while True:
        ...
        data = data.split('-')
        leik_madur2.rect.x = int(data[0])
        leik_madur2.rect.y = int(data[1])
...
while keyrir:
    for event in pygame.event.get():
        ...
        if connection_established:
            xStada = leik_madur.rect.x
            yStada = leik_madur.rect.y
            print(xStada, yStada)

            if event.type == pygame.KEYDOWN and event.key == pygame.K_UP:
                leik_madur.rect.y -= 10
                send_data = '{}-{}-{}'.format(xStada, yStada, stadsetning).encode()
                conn.send(send_data)
        ...
        texti_osigur = letur.render('Leikmaður2 fann Pikachu :(', False, (0, 0, 0))
